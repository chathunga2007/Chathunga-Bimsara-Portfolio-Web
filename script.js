...
// ===== FIXED ENGAGEMENT COUNTERS (Views + Likes) =====
const COUNTAPI_NAMESPACE = "chathunga-bimsara-portfolio-v2"; // නව namespace එකක් (පැරණි එක reset වුණා නම් හොඳයි)
...
async function fetchWithFallback(url, fallbackValue) {
  try {
    ...
    console.log("CountAPI temporarily unavailable, using cached value");
    return fallbackValue;
  }
}

async function initCounters() {
  // 1. LocalStorage එකෙන් පෙර count එක ගන්න (refresh වුණත් නැති වෙන්නේ නෑ)
  let cachedViews = parseInt(localStorage.getItem("portfolioViews")) || 1247;
  let cachedLikes = parseInt(localStorage.getItem("portfolioLikes")) || 89;

  // DOM එකට පළමුව පෙන්වන්න (ඉක්මනින් පේන්න)
  document.getElementById("viewCount").textContent = cachedViews.toLocaleString();
  const likeEl = document.getElementById("likeCount");
  const likeFooter = document.getElementById("likeCountFooter");
  if (likeEl) likeEl.textContent = cachedLikes.toLocaleString();
  if (likeFooter) likeFooter.textContent = cachedLikes.toLocaleString();

  // 2. Real CountAPI එකෙන් update කරන්න (background එකේ)
  try {
    // Views - every visit එකට increment වෙනවා
    const viewsData = await fetchWithFallback(
      `https://api.countapi.xyz/hit/${COUNTAPI_NAMESPACE}/views`,
      cachedViews
    );
    document.getElementById("viewCount").textContent = viewsData.toLocaleString();
    localStorage.setItem("portfolioViews", viewsData);

    // Likes - current value ගන්නවා
    const likesData = await fetchWithFallback(
      `https://api.countapi.xyz/get/${COUNTAPI_NAMESPACE}/likes`,
      cachedLikes
    );
    if (likeEl) likeEl.textContent = likesData.toLocaleString();
    if (likeFooter) likeFooter.textContent = likesData.toLocaleString();
    localStorage.setItem("portfolioLikes", likesData);
  } catch (e) {
    console.log("Using cached counters");
  }
}

// Flying hearts (පෙර තිබුණු එකම)
function createFlyingHearts() {
  for (let i = 0; i < 35; i++) {
    ...
    heart.animate([
      ...
      { transform: `translateY(-${window.innerHeight + 300}px) rotate(${Math.random() * 80 - 40}deg)`, opacity: 0 }
...
// Heart button (like කළාම global වැඩි වෙනවා)
function setupHeartButton() {
  const heartBtn = document.getElementById("heartToggle");
  let hasLiked = localStorage.getItem("hasLikedPortfolio") === "true";

  if (hasLiked) heartBtn.style.background = "#9ca3af";

  heartBtn.addEventListener("click", async () => {
    if (hasLiked) return;

    try {
      const res = await fetch(`https://api.countapi.xyz/hit/${COUNTAPI_NAMESPACE}/likes`);
      const data = await res.json();

      const newCount = data.value;
      document.getElementById("likeCount").textContent = newCount.toLocaleString();
      if (document.getElementById("likeCountFooter")) {
        document.getElementById("likeCountFooter").textContent = newCount.toLocaleString();
      }
      ...
      heartBtn.style.background = "#9ca3af";
    } catch (e) {
      createFlyingHearts(); // animation එක වැඩ කරනවා
    }
  });
}
